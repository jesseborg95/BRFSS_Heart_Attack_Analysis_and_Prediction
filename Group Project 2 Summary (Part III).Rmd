...
#### 5.2.1 SMART Question: Which variables are the most important variables for determining a heart attacks and which models provide the greatest accuracy for this prediction?
...
#### 5.3.1 SMART Question:

The key variables identified in trees in order of significance: agina, employment status, gen health, high bp. These are the variables which will be used in the logistic regression model using the forward selection method. The backward selection method was attempted, but since there are so many variables, the model takes too long or just wont run all together. We decided it would be more sensible to go with the forward selection method using the most important variables as determined by the trees. 
...
#### 5.4.1 SMART Question: Which is the best model to use for predicting heart attacks?

The table below shows the values for the different statistics for the different models explored:
