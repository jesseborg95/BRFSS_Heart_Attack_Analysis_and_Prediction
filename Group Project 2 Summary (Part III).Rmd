...
We observed the range for accuracy across all four models is 77.6% - 80.13%. The Random Forest Model produced the highest accuracy (80.13%) while the Classification Tree had the least accuracy (77.6%). 

However, in a classification task like predicting heart attack (and most health related cases), it is important to look beyond accuracy in selecting the best model. The recall is an important metric since it measures heart attack predictions identified correctly which is desirable, because we do not want to give a wrong prediction for patients using the our model. The logistic regression model as well as the Random Forrest Model gives the highest values for recall of 81.45% and 81.09% respectively.

Overall, the predictive models for heart attack had good prediction eerformance with only slight differences.

<br></br>

## 7. Conclusion & Key Takeaways

In this study, we built predictive models for heart attack using multiple machine learning algorithms, including logistic regression, decision tree, bagged tree, and random forest. By comparing their prediction performance on the test data set, our predictive models showed similar performance in predicting heart attack in terms of sensitivity, specificity, precision, recall and accuracy. However, the random forest prediction model had the highest accuracy while the Logistic regression model had the highest sensitivity (recall). 
...
An important limitation of our study is that BRFSS data were self-reported and subject to recall bias that could affect the performance of our predictive models. In the event that we have clinical data, we may experience better performance with our models for predicting heart attack.

Our models and findings can find application in early detention as well as prevention of heart attacks for the demography under study.
