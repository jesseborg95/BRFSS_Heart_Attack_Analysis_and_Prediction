...
After carrying out the Chi-Squared Test of Indepence to determine which categorical variables to use in our models, its time to build the models. Start off by using logistic regression modelling.
  
### Logistic Regression

NEED TO NARROW DOWN TRAINING SET?

Start by creating a training set and a test set for the model with a ratio of 70% training data, 30% test data.
...
Run the logistic regression model using the training data
...
```{r}
# decision trees
# Load required packages for decision trees
loadPkg('rpart')
loadPkg('rattle')

# Create first model using all the variables against heart attack
...
The plot can be made to look nicer by loading the 'rattle' package and using the 'fancyRpartPlot' function.

```{r}
# Load required package
loadPkg('rattle')

# Make the decision tree plot look nicer
fancyRpartPlot(heart_attack_fit1)
```

I need to carry on from here but I need to ask you guys about some things.
